...
pub async fn login_and_sync(homeserver_url: String) -> anyhow::Result<()> {
    ...
    if let Some(session) = auth.session() {
        auth.restore_session(session).await?;
    ...
    let login_types = auth.get_login_types().await?.flows;
...
async fn on_room_message(event: OriginalSyncRoomMessageEvent, room: Room) {
    ...
    println!("Room: {}, {name}: {}", room.name().unwrap(), msgtype.body);
}
